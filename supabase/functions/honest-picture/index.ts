...
${nzDataBlock || "(none — nz_signal must speak generally about the role category, no NZ percentages or NZ employer claims)"}`;
    ...
    // ---- Tasks call (await the promise we kicked off in parallel earlier) ----
    const tasksResult = await tasksPromise;
    const tRespOrNull: Response | null =
      tasksResult instanceof Response ? tasksResult : null;
    if (!tRespOrNull) {
      console.error("[honest-picture] tasks gateway threw before response", {
        error: (tasksResult as { __error: unknown }).__error,
      });
    }
    const tResp: Response = tRespOrNull ?? new Response("null", { status: 599 });
